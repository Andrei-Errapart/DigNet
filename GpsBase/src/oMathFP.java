...
public abstract class oMathFP {
     ...
     * Converts a FP to a string.
     *
     * Same as toString(x, 0, max_possible_digits)
     *
     * @param x the FP to convert
    ...
    public static void setPrecision(int p) {
        ...
        for (i = 0; i < sk.length; i++) SK[i] = (p <= sk_precision) ? sk[i] >> (sk_precision-p) : sk[i] << (p-sk_precision);
        for (i = 0; i < as.length; i++) AS[i] = (p <= as_precision) ? as[i] >> (as_precision-p) : as[i] << (p-as_precision);
        ...
        for (i = 0; i < lg.length; i++) LG[i] = (p <= lg_precision) ? lg[i] >> (lg_precision-p) : lg[i] << (p-lg_precision);
        for (i = 0; i < exp_p.length; i++) EXP_P[i] = (p <= exp_p_precision) ? exp_p[i] >> (exp_p_precision-p) : exp_p[i] << (p-exp_p_precision);
        frac_mask = ONE-1;
